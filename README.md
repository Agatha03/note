...
<form name="loginForm"(表单名字) action="a.jsp"(表示表单提交给谁去处理) method="get"(或者是"post) /form> get(在URL中可以知道相关信息如搜索关键词等)、 post

2、 使用Table设置表单

<tr></tr>：一行、<td></td>:一行中的一项，如标签；<td><input type="text"name="username"/></td> //name可以实现验证功能，name中type对应的是类型，如text类型，password类型。
...
这个指令等同于request.getRequestDispatcher().forward(request,response);
